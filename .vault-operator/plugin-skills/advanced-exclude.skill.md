---
id: advanced-exclude
name: Advanced Exclude
source: vault-native
plugin-type: community
status: enabled
class: NONE
description: "Enhances the Excluded files setting bringing gitignore syntax."
has-settings: false
needs-setup: true
commands:
  - id: "advanced-exclude:open-settings"
    name: "Advanced Exclude: Open settings"
---
...
- `advanced-exclude:open-settings` -- Advanced Exclude: Open settings

## Configuration File

Settings path: `.obsidian/plugins/advanced-exclude/data.json`

To configure this plugin programmatically:
1. Read the config: read_file(".obsidian/plugins/advanced-exclude/data.json")
2. Understand the settings structure and modify values as needed
3. Write changes: write_file(".obsidian/plugins/advanced-exclude/data.json", updatedJSON)
...
read_file(".vault-operator/plugin-skills/advanced-exclude.readme.md")

## Usage

When the user asks for functionality related to Advanced Exclude:
1. Read the plugin documentation (.readme.md) to understand capabilities and dependencies
2. Read the config file (.obsidian/plugins/advanced-exclude/data.json). If it does not exist, that is normal -- create it with the required settings
