...
export function ProductImageUploader({
  ...
  const upload = async () => {
    ...
    try {
      ...
      const up = await supabase.storage.from(RESEARCH_IMAGE_BUCKET).upload(object_path, file, {
        upsert: false,
        cacheControl: "3600",
      ...
      const res = await fetch(`/api/research-products/admin/${productId}/images`, {
        method: "POST",
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="text-xs text-[hsl(var(--muted-foreground))]">
        Uploads to: <code>{RESEARCH_IMAGE_BUCKET}/products/[productId]/</code>
      </div>
    </div>
  );
}