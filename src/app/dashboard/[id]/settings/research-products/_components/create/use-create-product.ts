...
export function useCreateProduct(onOpenChange: (v: boolean) => void, onCreated: () => void) {
  ...
  // Product-level research-chemical fields (replaced the shop-inherited
  // "material"/"made_in" text fields — those weren't real columns on
  // research_products, so creating a product with them set always errored).
  const [brand, setBrand] = useState("");
  const [casNumber, setCasNumber] = useState("");
  const [purity, setPurity] = useState("");
  const [researchUseOnly, setResearchUseOnly] = useState(true);
  ...
  const create = async () => {
    ...
    try {
      const res = await fetch("/api/research-products/admin", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          purity_percent: purity.trim() ? Number(purity) : null,
          research_use_only: researchUseOnly,
          price_cents: cents,
          status: "draft"
        }),
      });
      ...
      try {
        if (variants.length > 0) {
          ...
          for (const variant of variants) {
            ...
            const vRes = await fetch(`/api/research-products/admin/${productId}/variants`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
        ...
        if (productId) {
          await fetch(`/api/research-products/admin/${productId}`, { method: "DELETE" });
        }
        throw new Error(`Product creation rolled back - ${variantError.message}`);
      }

      await Promise.all([
        ...selectedCategoryIds.map(id => fetch(`/api/research-products/admin/${productId}/categories`, {
          method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ category_id: id })
        })),
        ...selectedCollectionIds.map(id => fetch(`/api/research-products/admin/${productId}/collections`, {
      ...
      if (images.length) {
        ...
        for (let i = 0; i < images.length; i++) {
          ...
          const { error } = await supabase.storage.from(RESEARCH_IMAGE_BUCKET).upload(path, webpFile, {
            upsert: false,
            cacheControl: "3600",
          ...
          await fetch(`/api/research-products/admin/${productId}/images`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
  ...
  return {
    state: {
      title, slug, baseSku, price, description, brand, casNumber, purity, researchUseOnly, images, availableSizes, availableColors,
    ...
    actions: {
      setTitle, setSlug, setBaseSku, setPrice, setDescription, setBrand, setCasNumber, setPurity, setResearchUseOnly, autoSlug, autoBaseSku,
