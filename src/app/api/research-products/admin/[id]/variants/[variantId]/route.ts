...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  if (wantsInventoryUpdate) {
    ...
    // Snapshot for the back-in-stock check below — only meaningful if this
    // request is actually changing quantity.
    if ("stock_on_hand" in input || "quantity" in input) {
      const { data: existingInv } = await supabase
        ...
        .maybeSingle();
      quantityBefore = existingInv?.quantity ?? 0;
    }
  ...
  if (quantityBefore !== null && inventoryRow) {
    // Admin client — see note in research-inventory/movements/route.ts:
    // pending signups belong to arbitrary customers, and RLS here only
    // exposes a user's own rows.
    await notifyBackInStockIfRestocked(createAdminClient(), variantId, quantityBefore, inventoryRow.quantity ?? 0);
  }
