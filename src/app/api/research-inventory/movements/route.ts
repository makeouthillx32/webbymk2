...
import { notifyBackInStockIfRestocked } from "@/lib/research/stockNotifications";
...
/**
 * POST /api/research-inventory/movements
 *
...
export async function POST(req: NextRequest) {
  ...
  // Snapshot quantity BEFORE the movement so we can tell whether this write
  // crosses the zero -> positive boundary (for back-in-stock notifications).
  ...
  // Admin client — notified signups can belong to any customer, and RLS on
  // research_stock_notifications only lets a user read/see their own rows,
  // so the auth-scoped `supabase` above can't see or clear other people's
  // pending signups.
  await notifyBackInStockIfRestocked(createAdminClient(), variant_id, quantityBefore, inventory?.quantity ?? 0);
