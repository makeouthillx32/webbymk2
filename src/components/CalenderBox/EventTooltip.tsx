// Enhanced EventTooltip.tsx - Fixed positioning and mobile responsiveness
'use client';
...
const EventTooltip = ({
  ...
  const getPositionClasses = () => {
    if (isMobile) {
      return {
        container: "fixed inset-x-2 bottom-4 z-50",
        arrow: "hidden"
      };
    }
  ...
  return (
    <>
      ...
      >
        ...
        <div className="bg-[hsl(var(--popover))] border border-[hsl(var(--border))] rounded-lg shadow-lg overflow-hidden">
          {/* Header */}
          ...
          <div className="max-h-80 overflow-y-auto">
            ...
            {/* Quick Actions for clicked dates */}
            {tooltipType === 'clicked-date' && (
              <div className="p-3">
                ...
                <div className="flex gap-2 flex-wrap">
                  {userRole === 'coachx7' && (
                    <button className="flex items-center space-x-1 px-3 py-2 text-xs bg-blue-50 text-blue-700 rounded-md hover:bg-blue-100 transition-colors">
                      <Clock className="w-3 h-3" />
                      <span>Log Hours</span>
                    </button>
                  )}
                  {(userRole === 'admin1' || userRole === 'coachx7') && (
                    <button className="flex items-center space-x-1 px-3 py-2 text-xs bg-green-50 text-green-700 rounded-md hover:bg-green-100 transition-colors">
                      <Calendar className="w-3 h-3" />
                      <span>Schedule Event</span>
                    </button>
                  )}
                  {userRole === 'admin1' && (
                    <button className="flex items-center space-x-1 px-3 py-2 text-xs bg-purple-50 text-purple-700 rounded-md hover:bg-purple-100 transition-colors">
