...
async function installFromGitHub(
  ...
  const gitUrl = isEnvTruthy(process.env.UNENTER_REMOTE)
 ...
 * UNENTER_REMOTE) or any URL that passes validateGitUrl (https, http,
 * file, git@ ssh).
 */
function resolveGitSubdirUrl(url: string): string {
  if (/^[a-zA-Z0-9-_.]+\/[a-zA-Z0-9-_.]+$/.test(url)) {
    return isEnvTruthy(process.env.UNENTER_REMOTE)
 ...
 *
 * UNENTER_SYNC_PLUGIN_INSTALL=1 delegates to the full loader — that
...
export const loadAllPluginsCacheOnly = memoize(
  async (): Promise<PluginLoadResult> => {
    if (isEnvTruthy(process.env.UNENTER_SYNC_PLUGIN_INSTALL)) {
