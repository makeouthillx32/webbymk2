/**
 * @deprecated REFERENCE CODE from Claude Code — DO NOT USE.
 *
 * This file is retained as a design reference for the tmux-based terminal
 * panel approach. It does NOT work in this project because:
 *   1. It imports from ../ink/instances.js (Claude Code internal)
 *   2. It requires tmux (not available on Windows)
 *   3. The Ink instance tracker API doesn't exist here
 *
 * For the working pop-out implementation, see terminalPopout.ts.
 *
 * Built-in terminal panel toggled with Meta+J.
 *
 * Uses tmux for shell persistence: a separate tmux server with a per-instance
 * socket (e.g., "unt-panel-a1b2c3d4") holds the shell session. Each unt
 ...
 * TUI while the shell keeps running. Next toggle re-attaches to the
 * same session.
 *
 ...
 * Uses a unique socket per TUI instance (based on session ID)
 * so that each instance has its own isolated terminal panel.
 */
export function getTerminalPanelSocket(): string {
  // Use first 8 chars of a UUID for uniqueness while keeping name short
  const sessionId = randomUUID()
  return `unt-panel-${sessionId.slice(0, 8)}`
}
...
class TerminalPanel {
  ...
  private createSession(): boolean {
    ...
    // Bind Meta+J (toggles back to TUI from inside the terminal)
    ...
    spawnSync('tmux', [
      ...
      'set-option', '-g', 'status-right', ' Alt+J to return to unt ', ';',
  ...
  private showShell(): void {
    ...
    if (!inkInstance) {
      console.error('Terminal panel: no Ink instance found, aborting')
      return
    }

    inkInstance.setAltScreenActive(true)
    try {
      ...
      inkInstance.setAltScreenActive(false)
    }
  }
