...
async function main() {
  ...
  console.log(`Resetting advanced_config on ${hosts.length} proxy host(s) to restore Nginx health...\n`);
  ...
  for (const host of hosts) {
    ...
    try {
      ...
      if (isOnline) {
        console.log(`✓ [${host.id}] ${domains} — CLEAN`);
        successCount++;
      } else {
        console.error(`⚠ [${host.id}] ${domains} — ${updated.meta?.nginx_err || "offline"}`);
      }
    } catch (err) {
  ...
  console.log(`\nRestore complete: ${successCount}/${hosts.length} hosts clean & online.`);
}
